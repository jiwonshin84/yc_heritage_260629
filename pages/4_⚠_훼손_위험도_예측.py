...
# ==========================================================
# 4. 입력 프레임 하단에 최근 3일 검증용 데이터 수동 강제 주입
# ==========================================================
curr_env = {}
curr_raw = {}
tm, data_time = "-", "-"
...
if len(merged_curr) >= 1:
    ...
    # 정렬 완료된 전체 프레임 중 가장 최신 데이터 3건을 화면 출력용으로 슬라이싱
...
# [★추가 파트★] 기상환경/대기오염현황 카드 밑에 3일치 모든 데이터를 표로 표시
st.write("")
st.markdown("#### 📅 분석 데이터셋 로그 (최근 3일 원본 데이터 확인)")
if not three_days_display.empty:
    st.dataframe(
        three_days_display[["date", "temp", "humidity", "rainfall", "wind", "pm10", "pm25", "so2", "no2", "co", "o3"]],
        column_config={
            "date": "측정 날짜", "temp": "기온 (°C)", "humidity": "습도 (%)", 
            "rainfall": "강수량 (mm)", "wind": "풍속 (m/s)", "pm10": "PM10", "pm25": "PM2.5"
        },
        use_container_width=True,
        hide_index=True
    )
else:
    st.warning("3일치 환경 데이터를 구성하지 못했습니다.")
